...
use production_nodes_types::pathfinder_types::types::block_hash::{
    calculate_event_commitment, calculate_receipt_commitment, calculate_transaction_commitment, compute_final_hash,
};
use production_nodes_types::pathfinder_types::types::gateway_state_update::{BlockStateUpdate, StarknetVersion};
use production_nodes_types::pathfinder_types::types::state_update::{state_diff_commitment::compute, StateUpdate};
...
impl StateUpdateBlockNumberStateMachine<Ok> {
    pub fn new() -> Self {
        Self { path: "/feeder_gateway/get_state_update?blockNumber".to_string(), state: Ok }
    }

    pub fn to_invalid(self) -> StateUpdateBlockNumberStateMachine<Invalid> {
        StateUpdateBlockNumberStateMachine { path: self.path, state: Invalid }
    }

    pub fn to_skipped(&self) -> StateUpdateBlockNumberStateMachine<Skipped> {
        StateUpdateBlockNumberStateMachine { path: self.path.clone(), state: Skipped }
    }
}
...
impl StateUpdateBlockNumberStateMachine<Invalid> {
    pub fn to_skipped(self) -> StateUpdateBlockNumberStateMachine<Skipped> {
        StateUpdateBlockNumberStateMachine { path: self.path, state: Skipped }
    }
}

impl StateUpdateBlockNumberStateMachine<Skipped> {
    pub fn to_okk(self) -> StateUpdateBlockNumberStateMachine<Ok> {
        StateUpdateBlockNumberStateMachine { path: self.path, state: Ok }
    }
}
...
impl StateMachine for StateUpdateBlockNumberStateMachineWrapper {
    fn run(&mut self, request_body: String, response_body: String, path: String) -> StateMachineResult {
        if self.filter(path) {
            ...
            *self = StateUpdateBlockNumberStateMachineWrapper::Skipped(match self {
                ...
                StateUpdateBlockNumberStateMachineWrapper::Invalid(machine) => machine.clone().to_skipped(),
    ...
    fn filter(&self, path: String) -> bool {
        fn validate_regex(input_path: String, machine_path: String) -> bool {
            if input_path.starts_with(&machine_path) {
                let re = Regex::new(r"^/feeder_gateway/get_state_update\?blockNumber=\d+&includeBlock=true$").unwrap();
        ...
        match self {
            StateUpdateBlockNumberStateMachineWrapper::Ok(machine) => validate_regex(path, machine.path.clone()),
            StateUpdateBlockNumberStateMachineWrapper::Invalid(machine) => validate_regex(path, machine.path.clone()),
            StateUpdateBlockNumberStateMachineWrapper::Skipped(machine) => validate_regex(path, machine.path.clone()),
        }
    }
    ...
    fn step(&mut self, _request_body: String, response_body: String) -> StateMachineResult {
        *self = match self {
            StateUpdateBlockNumberStateMachineWrapper::Ok(machine) => {
                match serde_json::from_str::<CheckVersion>(&response_body.clone()) {
                    JsonResult::Ok(version) => {
                        let pre_v_0_13_2 = version.block.starknet_version < StarknetVersion::V_0_13_2;

                        if pre_v_0_13_2 {
                            StateUpdateBlockNumberStateMachineWrapper::Skipped(machine.clone().to_skipped())
                        } else {
                                ...
                                JsonResult::Ok(block_state_machine) => {
                                    let valid_receipt_commitment = match compute_receipt_commitment(
                                        ...
                                        std::result::Result::Ok(commitment) => {
                                            commitment == block_state_machine.block.receipt_commitment
                                        }
                                        Err(_) => false,
                                    };

                                    let valid_transaction_commitment = match calculate_transaction_commitment(
                                        &block_state_machine.clone().block.transactions,
                                    ) {
                                        std::result::Result::Ok(commitment) => {
                                            commitment == block_state_machine.block.transaction_commitment
                                        }
                                        Err(_) => false,
                                    };
                                    ...
                                    let valid_state_diff_commitment =
                                        compute_state_diff(block_state_machine.clone().state_update)
                                            == block_state_machine.block.state_diff_commitment;

                                    let is_valid_block_hash =
                                    ...
                                    if valid_hashes {
                                        ...
                                        StateUpdateBlockNumberStateMachineWrapper::Invalid(machine.clone().to_invalid())
                                    }
                                }
                                Err(_) => {
                                    StateUpdateBlockNumberStateMachineWrapper::Invalid(machine.clone().to_invalid())
                    ...
                    Err(_) => StateUpdateBlockNumberStateMachineWrapper::Invalid(machine.clone().to_invalid()),
                }
            }
        ...
        match self {
            StateUpdateBlockNumberStateMachineWrapper::Ok(_) => {
                StateMachineResult::Ok("State Update by block number request SUCCESSFUL".to_string())
            }
...
pub fn compute_receipt_commitment(transaction_receipts: Vec<(Receipt, Vec<Event>)>) -> Result<Felt> {
    let receipts: Vec<Receipt> = transaction_receipts.into_iter().map(|(receipt, _events)| receipt).collect();
