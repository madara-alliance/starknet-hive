...
use starknet_devnet_types::rpc::transactions::{
    DeclareTransactionTrace, DeployAccountTransactionTrace, ExecutionInvocation, FunctionInvocation,
    InvokeTransactionTrace, L1HandlerTransactionTrace, TransactionTrace, TransactionType,
};
...
fn get_execute_call_info<S: StateReader>(
    ...
    Ok(match &execution_info.execute_call_info {
        Some(call_info) => match call_info.execution.failed {
            false => ExecutionInvocation::Succeeded(FunctionInvocation::try_from_call_info(call_info, state)?),
            true => ExecutionInvocation::Reverted(starknet_devnet_types::rpc::transactions::Reversion {
                revert_reason: execution_info.revert_error.clone().unwrap_or("Revert reason not found".into()),
            }),
        },
        None => match execution_info.revert_error.clone() {
            Some(revert_reason) => {
                ExecutionInvocation::Reverted(starknet_devnet_types::rpc::transactions::Reversion { revert_reason })
            }
            None => {
...
pub(crate) fn create_trace<S: StateReader>(
    ...
    let fee_transfer_invocation = get_call_info_invocation(state, &execution_info.fee_transfer_call_info)?;
    ...
    match tx_type {
        ...
        TransactionType::DeployAccount => Ok(TransactionTrace::DeployAccount(DeployAccountTransactionTrace {
            validate_invocation,
            constructor_invocation: get_call_info_invocation(state, &execution_info.execute_call_info)?,
            fee_transfer_invocation,
            state_diff,
            execution_resources,
        })),
        ...
        TransactionType::L1Handler => match get_call_info_invocation(state, &execution_info.execute_call_info)? {
            Some(function_invocation) => Ok(TransactionTrace::L1Handler(L1HandlerTransactionTrace {
                function_invocation,
                state_diff,
                execution_resources,
            })),
