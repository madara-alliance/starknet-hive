...
pub fn add_declare_transaction(
    ...
    if broadcasted_declare_transaction.is_max_fee_zero_value() {
        return Err(Error::MaxFeeZeroError { tx_type: broadcasted_declare_transaction.to_string() });
    }
    ...
    let (declare_transaction, contract_class) = match broadcasted_declare_transaction {
        BroadcastedDeclareTransaction::V1(ref v1) => {
            let declare_transaction =
                Transaction::Declare(DeclareTransaction::V1(DeclareTransactionV0V1::new(v1, class_hash)));

            (declare_transaction, v1.contract_class.clone().into())
        }
        BroadcastedDeclareTransaction::V2(ref v2) => {
            let declare_transaction =
                Transaction::Declare(DeclareTransaction::V2(DeclareTransactionV2::new(v2, class_hash)));

            (declare_transaction, v2.contract_class.clone().into())
        }
        BroadcastedDeclareTransaction::V3(ref v3) => {
            let declare_transaction =
                Transaction::Declare(DeclareTransaction::V3(DeclareTransactionV3::new(v3, class_hash)));
    ...
    starknet.handle_dump_event(DumpEvent::AddDeclareTransaction(broadcasted_declare_transaction))?;
