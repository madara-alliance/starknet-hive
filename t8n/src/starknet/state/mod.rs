...
use blockifier::{
    ...
    transaction::{
        errors::TransactionPreValidationError, objects::TransactionExecutionInfo, transactions::ExecutableTransaction,
    },
};
use constants::{
    CHARGEABLE_ACCOUNT_ADDRESS, CHARGEABLE_ACCOUNT_PRIVATE_KEY, DEVNET_DEFAULT_CHAIN_ID, DEVNET_DEFAULT_DATA_GAS_PRICE,
...
use starknet_devnet_types::{
    ...
    rpc::{
        ...
        transactions::{
            ...
            l1_handler_transaction::L1HandlerTransaction, BlockTransactionTrace, BroadcastedDeclareTransaction,
            BroadcastedDeployAccountTransaction, BroadcastedInvokeTransaction, BroadcastedTransaction,
            BroadcastedTransactionCommon, DeclareTransaction, SimulatedTransaction, SimulationFlag, Transaction,
...
impl Starknet {
    pub fn new(config: &StarknetConfig, acc_path: &Path) -> DevnetResult<Self> {
        ...
        for account_class_choice in [AccountContractClassChoice::Cairo0, AccountContractClassChoice::Cairo1] {
            let class_wrapper = account_class_choice.get_class_wrapper()?;
            state.predeclare_contract_class(class_wrapper.class_hash, class_wrapper.contract_class)?;
        }
        ...
        let accounts = predeployed_accounts
            .generate_accounts(acc_path, config.account_contract_class_hash, &config.account_contract_class)
            .unwrap();
        ...
        let chargeable_account =
            Account::new_chargeable(eth_erc20_fee_contract.get_address(), strk_erc20_fee_contract.get_address())?;
    ...
    fn next_block_timestamp(&mut self) -> BlockTimestamp {
        match self.next_block_timestamp {
            ...
            None => BlockTimestamp(
                (Starknet::get_unix_timestamp_as_seconds() as i64 + self.pending_block_timestamp_shift) as u64,
            ),
        }
    }
    ...
    pub(crate) fn handle_transaction_result(
        ...
        fn declare_contract_class(
            ...
            state.declare_contract_class(
                *class_hash,
                contract_class
                    .ok_or(Error::UnexpectedInternalError { msg: "contract class not provided".to_string() })?,
            )
        }
                ...
                if !tx_info.is_reverted() {
                    match &transaction.transaction {
                        Transaction::Declare(DeclareTransaction::V1(declare_v1)) => {
                            declare_contract_class(&declare_v1.class_hash, contract_class, &mut self.state)?
                        }
                        Transaction::Declare(DeclareTransaction::V2(declare_v2)) => {
                            declare_contract_class(&declare_v2.class_hash, contract_class, &mut self.state)?
                        }
                        Transaction::Declare(DeclareTransaction::V3(declare_v3)) => {
                            declare_contract_class(declare_v3.get_class_hash(), contract_class, &mut self.state)?
                        }
                        _ => {}
                    };
                }
    ...
    fn restart_pending_block(&mut self) -> DevnetResult<()> {
        ...
        block.header.l1_gas_price = GasPricePerToken {
            price_in_fri: GasPrice(self.block_context.block_info().gas_prices.strk_l1_gas_price.get()),
            price_in_wei: GasPrice(self.block_context.block_info().gas_prices.eth_l1_gas_price.get()),
        };
        block.header.l1_data_gas_price = GasPricePerToken {
            price_in_fri: GasPrice(self.block_context.block_info().gas_prices.strk_l1_data_gas_price.get()),
    ...
    pub fn call(
        ...
        let mut execution_context = blockifier::execution::entry_point::EntryPointExecutionContext::new(
        ...
        let res =
            call.execute(&mut transactional_state, &mut Default::default(), &mut execution_context).map_err(|err| {
    ...
    pub async fn mint(
        ...
        let signer = starknet_rs_signers::LocalWallet::from(starknet_rs_signers::SigningKey::from_secret_scalar(
        ...
        // apply the invoke tx
        add_invoke_transaction::add_invoke_transaction(self, BroadcastedInvokeTransaction::V1(invoke_tx))
    }
    ...
    pub fn abort_blocks(&mut self, starting_block_hash: Felt) -> DevnetResult<Vec<Felt>> {
        if self.config.state_archive != StateArchiveCapacity::Full {
            return Err(Error::UnsupportedAction {
                msg: ("The abort blocks feature requires state-archive-capacity set to full.".into()),
            });
        }
        ...
        let genesis_block = self.blocks.get_by_block_id(&BlockId::Number(genesis_block_number)).unwrap();
        ...
        let mut next_block_to_abort_hash =
            self.blocks.last_block_hash.ok_or(Error::UnsupportedAction { msg: "No blocks to abort".into() })?;
        ...
        while !reached_starting_block {
            ...
            if let Some(block) = block_to_abort {
                ...
                for tx_hash in block.get_transactions() {
                    ...
                    tx.execution_result = ExecutionResult::Reverted { reason: "Block aborted manually".to_string() };
                }
        ...
        if reached_starting_block {
            let current_block = self.blocks.hash_to_block.get(&last_reached_block_hash).ok_or(Error::NoBlock)?;
            ...
            let reverted_state = self
                ...
                .ok_or(Error::NoStateAtBlock { block_id: BlockId::Number(current_block.block_number().0) })?;
    ...
    pub fn get_block_with_receipts(&self, block_id: BlockId) -> DevnetResult<Block> {
        ...
        for transaction_hash in block.get_transactions() {
            ...
            let common_field = match receipt {
                ...
                | TransactionReceipt::L1Handler(L1HandlerTransactionReceipt { ref mut common, .. })
            ...
            transaction_receipts.push(TransactionWithReceipt { receipt, transaction: transaction.transaction });
        }
    ...
    pub fn get_transaction_by_block_id_and_index(
        ...
        let transaction_hash =
            block.get_transactions().get(index as usize).ok_or(Error::InvalidTransactionIndexInBlock)?;
    ...
    pub fn get_transaction_traces_from_block(&self, block_id: &BlockId) -> DevnetResult<Vec<BlockTransactionTrace>> {
        ...
        if let Transactions::Full(txs) = transactions {
            for tx in txs {
                ...
                let block_trace = BlockTransactionTrace { transaction_hash: tx_hash, trace_root: trace };
    ...
    pub fn simulate_transactions(
        ...
        for broadcasted_transaction in transactions.iter() {
            let blockifier_transaction = broadcasted_transaction.to_blockifier_account_transaction(&chain_id)?;
            ...
            let state_diff: ThinStateDiff =
                StateDiff::generate(&mut transactional_state, &mut transactional_rpc_contract_classes)?.into();
        ...
        let estimated =
            estimations::estimate_fee(self, block_id, transactions, Some(!skip_fee_charge), Some(!skip_validate))?;
        ...
        let simulation_results = transactions_traces
            .into_iter()
            .zip(estimated)
            .map(|(trace, fee_estimation)| SimulatedTransaction { transaction_trace: trace, fee_estimation })
            .collect();
