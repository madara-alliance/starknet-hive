...
use super::v7::{
    accounts::{
        account::{
            Account, AccountError, ConnectedAccount, DeclarationV2, DeclarationV3, ExecutionEncoder, ExecutionV1,
    ...
    endpoints::{
        declare_contract::{
            extract_class_hash_from_error, get_compiled_contract_from_string, parse_class_hash_from_error, RunnerError,
        },
...
impl StarknetHive {
    pub async fn new(
        ...
        let account_data = create_account(&provider, AccountType::Oz, Option::None, Some(account_class_hash)).await?;
        ...
        let deployment_hash =
            deploy_account(&provider, chain_id, wait_config, account_data, DeployAccountVersion::V3).await?;
    ...
    pub async fn new_with_custom_account(
        ...
        let (flattened_sierra_class, compiled_class_hash) = get_compiled_contract_from_string(sierra, casm).await?;

        let custom_account_class_hash =
            match paymaster_account.declare_v3(flattened_sierra_class.clone(), compiled_class_hash).send().await {
                Ok(result) => {
        ...
        let account_data =
            create_account(&provider, AccountType::Oz, Option::None, Some(custom_account_class_hash?)).await?;
...
impl Account for StarknetHive {
    type SignError = <SingleOwnerAccount<JsonRpcClient<HttpTransport>, LocalWallet> as Account>::SignError;
