...
pub async fn mint(base_url: Url, mint_request: &MintRequest2) -> Result<MintResponse, MintError> {
    ...
    let response =
        Client::new().post(mint_url).header("Content-type", "application/json").json(mint_request).send().await?;

    if !response.status().is_success() {
        let status_code = response.status();
        let error_message = response.text().await.map_err(|_| MintError::ResponseTextError)?;
        Err(MintError::ResponseStatusError { status_code, message: Some(error_message) })
    } else {
        let mint_response = response.json::<MintResponse>().await.map_err(|_| MintError::ResponseParseError)?;
        Ok(mint_response)
    }
}
