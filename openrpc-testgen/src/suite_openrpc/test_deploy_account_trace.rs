...
use starknet_types_rpc::{BlockId, BlockTag, DeployAccountTransactionTrace, EntryPointType, TransactionTrace};
...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        ...
        let trace_result = account_paymaster.provider().trace_transaction(deploy_account_hash).await;
        ...
        assert_matches_result!(trace, TransactionTrace::DeployAccount(DeployAccountTransactionTrace { .. }));

        let deploy_acc_trace = match trace {
