...
use starknet_types_rpc::{BlockId, BlockTag, DeclareTransactionTrace, EntryPointType, TransactionTrace};
...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        ...
        let (flattened_sierra_class, compiled_class_hash) = get_compiled_contract(
            ...
            PathBuf::from_str(
                "target/dev/contracts_contracts_sample_contract_7_HelloStarknet.compiled_contract_class.json",
            )?,
        )
        .await?;
        ...
        let trace_result = account.provider().trace_transaction(declare_result.transaction_hash).await;
        ...
        assert_matches_result!(trace, TransactionTrace::Declare(DeclareTransactionTrace { .. }));
