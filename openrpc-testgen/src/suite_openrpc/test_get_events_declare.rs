...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        let (flattened_sierra_class, compiled_class_hash) = get_compiled_contract(
            PathBuf::from_str("target/dev/contracts_contracts_smpl13_HelloStarknet.contract_class.json")?,
            PathBuf::from_str("target/dev/contracts_contracts_smpl13_HelloStarknet.compiled_contract_class.json")?,
        )
        .await?;
        ...
        let declaration_result = sender.declare_v3(flattened_sierra_class.clone(), compiled_class_hash).send().await?;
        ...
        let first_event = events
            .events
            .first()
            .ok_or_else(|| OpenRpcTestGenError::Other("First event (declaration) not found".to_string()))?;
        ...
        assert_result!(
            events.events.len() == 1,
            format!("Invalid events count, expected {}, got {}", 1, events.events.len())
        );
        ...
        assert_result!(
            ...
            format!("Invalid fee (transfer) amount in event data, expected {}, got {:?}", Felt::ZERO, data.get(1))
        );
        ...
        assert_result!(
            ...
            format!("Invalid keccak transfer in event keys, expected {}, got {:?}", transfer_keccak, keys.first())
        );

        let sender_address = sender.address();
        assert_result!(
            keys.get(1) == Some(&sender_address),
            format!("Invalid sender address in event keys, expected {}, got {:?}", sender_address, keys.get(1))
        );
        ...
        assert_result!(
            ...
            format!("Invalid sequencer address in event keys, expected {}, got {:?}", sequencer_address, keys.get(2))
        );
