...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        ...
        let storage_proof = test_input
            ...
            .provider()
            .get_storage_proof(BlockId::Tag(BlockTag::Latest), Some(vec![declare_result.class_hash]), None, None)
            .await?;

        let merkle_tree =
            MerkleTree::from_proof(storage_proof.classes_proof, Some(storage_proof.global_roots.classes_tree_root));

        let expected_child = merkle_tree.compute_expected_child_for_class_proof(&compiled_class_hash);
