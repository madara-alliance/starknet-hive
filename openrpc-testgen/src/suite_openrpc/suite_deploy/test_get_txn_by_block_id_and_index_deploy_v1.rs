...
use starknet_types_rpc::{BlockId, BroadcastedInvokeTxn, BroadcastedTxn, InvokeTxn, MaybePendingBlockWithTxs, Txn};
...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        ...
        let (valid_signature, deploy_hash) = verify_invoke_v1_signature(
            &deploy_request,
            None,
            sender.clone().provider().chain_id().await?.to_hex_string().as_str(),
        )?;

        let deploy_result = test_input
            .random_paymaster_account
            .provider()
            .add_invoke_transaction(BroadcastedTxn::Invoke(BroadcastedInvokeTxn::V1(deploy_request)))
            .await?;
        ...
        assert_result!(
            ...
            format!("Expected seventh calldata element to be {:#?}, got {:#?}", unique_hex, calldata_seventh)
        );
        ...
        assert_result!(
            txn.max_fee == MAX_FEE,
            format!("Expected max fee to be {:#?}, got {:#?}", MAX_FEE, txn.max_fee)
        );
