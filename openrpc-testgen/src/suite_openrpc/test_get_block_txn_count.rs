...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        ...
        let (flattened_sierra_class, compiled_class_hash) = get_compiled_contract(
            PathBuf::from_str("target/dev/contracts_contracts_smpl5_HelloStarknet.contract_class.json")?,
            PathBuf::from_str("target/dev/contracts_contracts_smpl5_HelloStarknet.compiled_contract_class.json")?,
        )
        .await?;
        ...
        let factory = ContractFactory::new(declaration_result.class_hash, paymaster_account.clone());
        ...
        loop {
            let current_block_number = test_input.random_paymaster_account.provider().block_number().await?;
        ...
        let invoke_result = test_input.random_paymaster_account.execute_v3(calls).send().await?;
        ...
        assert_result!(
            block_txn_count == 1,
            format!("Mismatch in transaction count. Expected: {}, Found: {}.", 1, block_txn_count)
        );
