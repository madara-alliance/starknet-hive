...
use starknet_types_rpc::{
    BlockId, BlockStatus, BlockTag, DaMode, DeclareTxn, PriceUnit, TransactionAndReceipt, Txn, TxnFinalityStatus,
    TxnReceipt,
};
...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        let (flattened_sierra_class, compiled_class_hash) = get_compiled_contract(
            PathBuf::from_str("target/dev/contracts_contracts_smpl3_HelloStarknet.contract_class.json")?,
            PathBuf::from_str("target/dev/contracts_contracts_smpl3_HelloStarknet.compiled_contract_class.json")?,
        )
        .await?;
        ...
        let (valid_signature, declare_tx_hash) =
            verify_declare_v3_signature(&declare_v3_request, None, chain_id.to_hex_string().as_str())?;

        let signature = declare_v3_request.clone().signature;

        let class_and_tx_hash = prepared_declaration_v3.send_from_request(declare_v3_request).await?;
        ...
        assert_result!(
            ...
            format!("Mismatch expected: {:?}, but got: {:?}", BlockStatus::AcceptedOnL2, block_with_receipts.status)
        );

        let block_header = block_with_receipts.block_header;
        assert_result!(
            block_header.block_hash == block_hash_and_number.block_hash,
            format!("Mismatch block hash: {} != {}", block_header.block_hash, block_hash_and_number.block_hash)
        );

        assert_result!(
            block_header.block_number == block_hash_and_number.block_number,
            format!("Mismatch block number: {} != {}", block_header.block_number, block_hash_and_number.block_number)
        );
        ...
        assert_result!(
            ...
            format!("Mismatch gas price: {} != {}", block_header.l1_data_gas_price.price_in_wei, BLOB_GAS_PRICE)
        );

        assert_result!(
            block_header.l1_gas_price.price_in_fri == STRK_GAS_PRICE,
            format!("Mismatch l1 gas price: {} != {}", block_header.l1_gas_price.price_in_fri, STRK_GAS_PRICE)
        );

        assert_result!(
            block_header.l1_gas_price.price_in_wei == GAS_PRICE,
            format!("Mismatch gas price: {} != {}", block_header.l1_gas_price.price_in_wei, GAS_PRICE)
        );
        ...
        let declare_tx = match transaction {
            Txn::Declare(declare_tx) => match declare_tx {
                ...
                _ => {
                    return Err(OpenRpcTestGenError::UnexpectedTxnType("Expected Declare V3 Transaction.".to_string()));
                }
            },
            _ => {
                return Err(OpenRpcTestGenError::UnexpectedTxnType("Expected Declare Transaction.".to_string()));
            }
        };
        ...
        // Declare Txn
        assert_result!(declare_tx.account_deployment_data.is_empty(), "Expected account deployment data to be empty");

        assert_result!(
            declare_tx.class_hash == class_and_tx_hash.class_hash,
            format!("Expected class hash to be {:?}, got {:?}", class_and_tx_hash.class_hash, declare_tx.class_hash)
        );
        ...
        assert_result!(
            ...
            format!("Expected nonce to be {:?}, got {:?}", sender_nonce, declare_tx.nonce)
        );
        ...
        assert_result!(declare_tx.paymaster_data.is_empty(), "Expected paymaster data to be empty");

        let sender_address = sender.address();
        assert_result!(
            declare_tx.sender_address == sender_address,
            format!("Expected sender address to be {:?}, got {:?}", sender_address, declare_tx.sender_address)
        );
        ...
        assert_result!(
            ...
            format!("Expected signature: {:?}, got {:?}", signature, declare_tx.signature)
        );
        ...
        assert_result!(
            ...
            format!("Expected tip to be {:?}, got {:?}", expected_tip, declare_tx.tip)
        );
        ...
        let declare_txn_gas_price_hex = Felt::from_dec_str(&DECLARE_TXN_GAS_PRICE.to_string())?.to_hex_string();
        assert_result!(
        ...
        assert_result!(
            declare_tx.resource_bounds.l2_gas.max_price_per_unit == expected_l2_gas_max_price_per_unit,
            format!(
                "Expected l2 gas max price per unit
                 to be {:?}, got {:?}",
                expected_l2_gas_max_price_per_unit, declare_tx.resource_bounds.l2_gas.max_price_per_unit
            )
        );
        ...
        assert_result!(
            ...
            format!("Expected overall fee to be {:?}, got {:?}", estimate_fee.overall_fee, actual_fee.unit)
        );

        assert_result!(
            actual_fee.unit == PriceUnit::Fri,
            format!("Expected price unit to be {:?}, got {:?}", PriceUnit::Fri, actual_fee.unit)
        );
        ...
        assert_result!(
            ...
            format!("Expected event from address to be {:?}, got {:?}", STRK_ADDRESS, event.from_address)
        );

        let event_data_first =
            *event.data.first().ok_or_else(|| OpenRpcTestGenError::Other("Missing first event data".to_string()))?;

        assert_result!(
            event_data_first == estimate_fee.overall_fee,
            format!("Expected first event data to be {:?}, got {:?}", estimate_fee.overall_fee, event_data_first)
        );

        let event_data_second =
            *event.data.get(1).ok_or_else(|| OpenRpcTestGenError::Other("Missing first event data".to_string()))?;

        let expected_event_second_data = Felt::ZERO;

        assert_result!(
            event_data_second == expected_event_second_data,
            format!("Expected second event data to be {:?}, got {:?}", expected_event_second_data, event_data_second)
        );

        let event_keys_first =
            *event.keys.first().ok_or_else(|| OpenRpcTestGenError::Other("Missing first event key".to_string()))?;
        let transfer_keccak = starknet_keccak("Transfer".as_bytes());
        assert_result!(
            event_keys_first == transfer_keccak,
            format!("Expected first event key to be {:?}, got {:?}", transfer_keccak, event_keys_first)
        );

        let event_keys_second =
            *event.keys.get(1).ok_or_else(|| OpenRpcTestGenError::Other("Missing second event key".to_string()))?;
        assert_result!(
            event_keys_second == sender_address,
            format!("Expected second event key to be {:?}, got {:?}", sender_address, event_keys_second)
        );

        let event_keys_third =
            *event.keys.get(2).ok_or_else(|| OpenRpcTestGenError::Other("Missing third event key".to_string()))?;
        assert_result!(
            event_keys_third == SEQUENCER_ADDRESS,
            format!("Expected third event key to be {:?}, got {:?}", SEQUENCER_ADDRESS, event_keys_third)
        );
        ...
        assert_result!(
            declare_receipt.common_receipt_properties.messages_sent.is_empty(),
