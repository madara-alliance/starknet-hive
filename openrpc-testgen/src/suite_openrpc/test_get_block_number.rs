...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        ...
        let block_number_after_first_tx = test_input.random_paymaster_account.provider().block_number().await?;
        ...
        let block_number_after_second_tx = test_input.random_paymaster_account.provider().block_number().await?;
