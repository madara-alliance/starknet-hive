use crate::{
    ...
    utils::v7::{
        accounts::{
            ...
            deployment::{
                deploy::{
                    deploy_account_v3_from_request, estimate_fee_deploy_account, get_deploy_account_request,
...
use starknet_types_rpc::{
    BlockId, BlockStatus, BlockTag, DaMode, DeployAccountTxn, PriceUnit, TransactionAndReceipt, Txn, TxnFinalityStatus,
    TxnReceipt,
};
...
impl RunnableTrait for TestCase {
    ...
    async fn run(test_input: &Self::Input) -> Result<Self, OpenRpcTestGenError> {
        ...
        let deploy_account_receipt = match receipt {
            ...
            _ => {
                return Err(OpenRpcTestGenError::UnexpectedTxnType("Expected Deploy Account Receipt.".to_string()));
            }
        };
        ...
        assert_result!(
            ...
            format!("Expected class hash to be {:?}, but got: {:?}.", input_class_hash, deploy_account_tx.class_hash)
        );

        let constructor_calldata_len = deploy_account_tx.constructor_calldata.len();
        assert_result!(
            constructor_calldata_len == 1,
            format!("Expected constructor calldata length to be 1, but got {}.", constructor_calldata_len)
        );

        let constructor_calldata = *deploy_account_tx
            .constructor_calldata
            .first()
            .ok_or_else(|| OpenRpcTestGenError::Other("Missing constructor calldata".to_string()))?;
        ...
        assert_result!(
            ...
            format!("Expected max fee to be {:?}, but got {:?}.", Felt::ZERO, deploy_account_tx.nonce)
        );
        ...
        assert_result!(
            ...
            format!("Expected paymaster data to be empty, but it was not. Got: {:?}", deploy_account_tx.paymaster_data)
        );

        let expected_tip = Felt::ZERO;
        assert_result!(
            deploy_account_tx.tip == expected_tip,
            format!("Expected tip to be {:?}, but got {:?}", expected_tip, deploy_account_tx.tip)
        );
        ...
        assert_result!(
            deploy_account_tx.resource_bounds.l1_gas.max_price_per_unit == deploy_account_txn_gas_price_hex,
            format!(
                "Expected l1 gas max price per unit
                 to be {:?}, but got {:?}",
                deploy_account_txn_gas_price_hex, deploy_account_tx.resource_bounds.l1_gas.max_price_per_unit
            )
        );
        ...
        assert_result!(
            deploy_account_tx.resource_bounds.l2_gas.max_price_per_unit == expected_l2_gas_max_price_per_unit,
            format!(
                "Expected l2 gas max price per unit
                 to be {:?}, but got {:?}",
                expected_l2_gas_max_price_per_unit, deploy_account_tx.resource_bounds.l2_gas.max_price_per_unit
            )
        );

        assert_result!(is_valid_signature, "Invalid signature for deploy account request, checked by t9n.");

        assert_result!(
            deploy_account_tx.signature == signature,
            format!("Expected signature: {:?}, got {:?}", signature, deploy_account_tx.signature)
        );

        // Deploy Account receipt
        let actual_fee = deploy_account_receipt.common_receipt_properties.actual_fee.clone();
        ...
        let events = deploy_account_receipt.common_receipt_properties.events.clone();
        ...
        let finality_status = deploy_account_receipt.common_receipt_properties.finality_status.clone();
        ...
        assert_result!(
            deploy_account_receipt.common_receipt_properties.messages_sent.is_empty(),
            "Expected no messages sent"
        );

        assert_result!(
            deploy_account_receipt.common_receipt_properties.transaction_hash == deploy_account_result.transaction_hash,
            format!(
                "Invalid transaction hash, expected {:?}, got {:?}",
                deploy_account_result.transaction_hash,
                deploy_account_receipt.common_receipt_properties.transaction_hash
            )
        );

        let execution_status = match deploy_account_receipt.common_receipt_properties.anon.clone() {
